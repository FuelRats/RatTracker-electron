export class RatEmitter {
	private readonly limited: boolean;
	private readonly __listeners: Array<Function | Object>;
	 ...
	 * @returns {void}
	 */
	constructor(limited: boolean, validEvents: string[]) {
		...
		if (limited) {
			if (
				!validEvents ||
				!Array.isArray(validEvents) ||
				!validEvents.length
			) {
				throw new Error(
					"Invalid constructor args. limiting valid events requires an array of event names."
				);
			}
	 ...
	 */
	on(evt: string, func: Function | Object): this {
		...
		if (this.limited && !this.__listeners.hasOwnProperty(evt)) {
			window.console.error(
				"WARN: Attempted registration of listener for invalid event name. Event: ",
				evt
			);
			return this;
	 ...
	 */
	once(evt: string, func: Function | Object): this {
		return this.on(evt, {
			func: func,
			once: true
		});
	 ...
	 * @return {Object}        Current instance.
	 */
	off(evt: string, func: Function | Object): this {
		...
		let listenerIndex = this.__listeners[evt].findIndex(
			(listener: { func: Function | Object; evt: string }) =>
				listener.func === func
		);
		if (listenerIndex < 0) {
			return this;
	 ...
	 * @return {Object}         Current instance.
	 */
	async _emitEvent(evt: string, ...args: any) {
