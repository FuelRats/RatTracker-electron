...
export class RatSocket extends RatEmitter {
    ...
    async connect(token : string) {
        ...
        return new Promise((resolve, reject) => {
            this.welcomeTimeout = window.setTimeout(() => {
                ...
                reject({
                    'errors': [ { 'code': 408, 'detail': 'No response from server', 'status': 'Request Timeout', 'title': 'Request Timeout' }],
                    'meta': {}
                });
            }, 60000);
            ...
            this.once('connection', onConnect)!
                .once('ratsocket:error', onError);

            this.socket = new WebSocket(`${RatConfig.WssUri}?bearer=${this.currentToken}`);
            this.socket.onopen = () => { this.connectionOpened(); };
            this.socket.onclose = (data) => { this.connectionClosed(data); };
            this.socket.onerror = (data) => { this.errorReceived(data); reject(data); };
    ...
    messageReceived(data : any) {
        ...
        if (_data.meta.reqId && this.openRequests.hasOwnProperty(_data.meta.reqId)) {
