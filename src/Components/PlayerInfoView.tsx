...
class PlayerInfoView extends React.Component<RatDataProps> {
  getStatusFlags(flags: number) {
    ...
    if (flags & EDStatusFlags.LandingGearDown) _flags.push("Landing gear down");
    if (flags & EDStatusFlags.ShieldsUp) _flags.push("Shields up");
    if (flags & EDStatusFlags.SuperCruise) _flags.push("Supercruise");
    if (flags & EDStatusFlags.FlightAssistOff) _flags.push("Flight Assist Off");
    ...
    if (flags & EDStatusFlags.SrvTurretView) _flags.push("SRV - Turret View");
    if (flags & EDStatusFlags.SrvTurretRetracted)
      _flags.push("SRV - Turret Retracted");
    if (flags & EDStatusFlags.SrvDriveAssist) _flags.push("SRV - Drive Assist");
    if (flags & EDStatusFlags.FsdMasslocked) _flags.push("FSD - Mass Locked");
    ...
    if (flags & EDStatusFlags.OverHeating) _flags.push("Overheating (> 100%)");
            ...
            <tbody>
              ...
              {_data.Player.Pos.Docked ? (
                <tr>
                  <td>Docked at</td>
                  <td align="right">
                    {_data.Player.Pos.Body} ({_data.Player.Pos.BodyType})
                  </td>
                </tr>
              ) : null}
              <tr>
                ...
                <td align="right">
                  ...
                  <br />
                  {_data.Player.CMDR.Ship} ({_data.Player.CMDR.ShipIdent})
                </td>
              </tr>
              <tr>
                <td>Gamemode</td>
              ...
              <tr>
                ...
                <td align="right">
                  {_data.Status != null && _data.Status.Flags != null
